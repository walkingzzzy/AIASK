    ...
    async def quant_manager(action: str, code: Optional[str] = None, **kwargs):
        ...
        try:
            ...
            if action == "help":
                ...
                supported = {"momentum", "value", "quality", "volatility", "reversal"}
                ...
                for stock_code in codes[:200]:  # cap at 200
                    try:
                        ...
                        if "reversal" in factors:
                            fv["reversal"] = float(-(closes[-1] - closes[-6]) / closes[-6]) if len(closes) >= 6 and closes[-6] else 0.0
                        ...
                        for stock_code, fv in results.items():
                            ...
                            try:
                                ...
                                if fname == "momentum":
                                    ...
                                    lagged_fv = float(-(lagged_closes[-1] - lagged_closes[-6]) / lagged_closes[-6]) if len(lagged_closes) >= 6 and lagged_closes[-6] else 0.0
                                else:
