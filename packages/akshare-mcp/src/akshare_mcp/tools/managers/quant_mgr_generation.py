...
async def _load_market_frame(
    *,
    ...
    bars: int,
    db: Any,
    data_source_get_kline_fn: Callable[..., Any],
    ...
    try:
        ...
        klines = []
    if not klines:
        ds_rows = data_source_get_kline_fn(one_code, period="daily", limit=max(120, int(bars)))
        if ds_rows:
    ...
    *,
    ...
    for one_code in scoped_codes:
        frame, one_source, reason = await _load_market_frame(
            ...
            bars=180,
            db=db,
            data_source_get_kline_fn=data_source_get_kline_fn,
...
async def handle_llm_factor_mining(
    *,
    ...
    candidate_count = max(1, min(int(kw.get("candidate_count", 8) or 8), 16))
