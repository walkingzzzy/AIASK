        ...
        if not acceptance_matrix.get("migration_ready"):
            blocker_details.append(
                _append(
                    ...
                    "在目标 Timescale 实例执行迁移与 backfill，并确认 migration key 全量落库。",
                )
            )
                ...
                row = await conn.fetchrow(
                    ...
                    SELECT
                        ...
                        COALESCE(
                            COUNT(*) FILTER (
                                WHERE COALESCE(source_type, '') = 'paper_execution_backfill'
                                   OR COALESCE(payload->>'backfill_mode', '') = 'paper_execution_native_backfill_v1'
                            ),
                            0
                        )::int AS backfilled_count,
                        COALESCE(
                            ...
                            0
                        )::int AS backfilled_signal_count,
                        COALESCE(
                            COUNT(*) FILTER (
                                WHERE NOT (
                        ...
                        COALESCE(
                            ...
                            0
                        )::int AS compile_stable_signal_count,
                        COALESCE(
                            COUNT(*) FILTER (WHERE COALESCE(proxy_only, FALSE)),
                            0
                        )::int AS proxy_only_count
                    FROM strategy_signal_evidence
