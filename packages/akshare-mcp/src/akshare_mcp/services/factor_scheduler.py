...
# ═══════════════════════════════════════════════════════════════════════════════
# Factor Mining Factory 委托桥接
# ═══════════════════════════════════════════════════════════════════════════════

_FACTORY_MINING_ENABLED = os.getenv("FACTOR_MINING_FACTORY_ENABLED", "0").strip().lower() in ("1", "true", "yes")


async def run_factory_mining_cycle(
    *,
    trigger: str = "scheduler_delegate",
    codes: Optional[List[str]] = None,
) -> dict:
    """委托给因子挖掘工厂执行挖掘周期。

    当 FACTOR_MINING_FACTORY_ENABLED=1 时，FactorScheduler.run_once() 完成
    ...
    try:
        ...
        logger.info(
            "FactorMiningFactory delegate: admitted=%d pool=%d",
            result.get("admitted_count", 0),
            result.get("pool_size", 0),
        )
        return result
    except Exception as exc:
        logger.warning("FactorMiningFactory delegate failed: %s", exc)
        return {"success": False, "error": str(exc), "delegate": True}


def is_factory_mining_enabled() -> bool:
    """检查因子挖掘工厂是否启用。"""
    return bool(
