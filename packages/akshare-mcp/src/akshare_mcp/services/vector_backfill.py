...
async def _load_legacy_vector_documents(
    ...
    *,
    ...
    idx = 2
    if stock_codes:
        where_clauses.append(f"stock_code = ANY(${idx}::text[])")
        params.append(stock_codes)
        idx += 1
    if doc_types:
        where_clauses.append(f"LOWER(COALESCE(doc_type, '')) = ANY(${idx}::text[])")
        params.append([str(item or "").strip().lower() for item in doc_types if str(item or "").strip()])
        idx += 1
    if start_date is not None:
        where_clauses.append(f"date >= ${idx}::date")
...
async def _load_legacy_research_reports(
    ...
    *,
    ...
    idx = 2
    if stock_codes:
        where_clauses.append(f"code = ANY(${idx}::text[])")
        params.append(stock_codes)
        idx += 1
    if start_date is not None:
        where_clauses.append(f"publish_date >= ${idx}::date")
        params.append(start_date)
        idx += 1
    if end_date is not None:
        where_clauses.append(f"publish_date <= ${idx}::date")
