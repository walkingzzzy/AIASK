...
def aggregate_trade_position(existing: Optional[dict], fills: list[dict]) -> dict:
    ...
    exit_commission = 0.0
    entry_trade_id = base.get("entry_trade_id")
    exit_trade_id = base.get("exit_trade_id")
    entry_order_id = base.get("entry_order_id")
    exit_order_id = base.get("exit_order_id")
    opened_at = base.get("opened_at")
    closed_at = base.get("closed_at")
    last_trade_time = base.get("last_trade_time")
    ...
    if entry_shares > 0 and remaining_shares > 0:
        status = "open"
