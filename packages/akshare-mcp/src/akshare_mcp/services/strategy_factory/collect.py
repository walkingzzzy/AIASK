...
class DataCollector:
    ...
    @classmethod
    def _event_strategy_preferences(cls, direction: str, theme_name: str, opportunity_hint: str) -> List[str]:
    ...
    def _default_theme_code(cls, cluster: dict[str, Any], fallback: str = "general") -> str:
        ...
        for item in raw_themes:
            if isinstance(item, dict):
                code = str(item.get("theme_code") or item.get("code") or item.get("theme") or "").strip()
            else:
    ...
    @classmethod
    async def _collect_event_driven_snapshot(cls, db) -> tuple[dict, str, Optional[str], Optional[dict]]:
        payload = {
            "enabled": False,
            "event_count": 0,
        ...
        for cluster in list(clusters or []):
            ...
            for item in list(cluster.get("themes") or []):
                if isinstance(item, dict):
                    theme_code = str(item.get("theme_code") or item.get("code") or item.get("theme") or "").strip()
                    theme_name = str(item.get("theme_name") or theme_code).strip() or theme_code
                    direction = str(item.get("direction") or cluster.get("direction") or "neutral").strip().lower() or "neutral"
                else:
                    theme_code = str(item or "").strip()
                    theme_name = str((theme_defs.get(theme_code) or {}).get("theme_name") or theme_code).strip() or theme_code
                    direction = str(cluster.get("direction") or "neutral").strip().lower() or "neutral"
                if theme_code:
                    grouped[theme_code] = {
            ...
            for signal in signal_rows:
                theme_code = str(signal.get("theme_code") or "").strip() or cls._default_theme_code(cluster)
                ...
                group = grouped.setdefault(
                    ...
                    {
                        "theme_code": theme_code,
                        "theme_name": str(theme_def.get("theme_name") or theme_code).strip() or theme_code,
                        "direction": direction,
                        "signal_rows": [],
                        "supporting_reasons": [],
                    },
                ...
                rationale = cls._compact_text(signal.get("rationale") or (signal.get("evidence") or {}).get("summary"), limit=100)
            ...
            for theme_code, theme in grouped.items():
                ...
                signal_values.sort(key=lambda item: float(item.get("final_score") or 0.0), reverse=True)
                final_scores = [cls._clip_score(item.get("final_score"), default=0.0) for item in signal_values]
                ...
                target_symbols = [item.get("symbol") for item in signal_values if item.get("symbol")][:5]
                opportunity_hint = "factor_acceleration" if "factor" in str(theme_code or "").lower() else "sector_breakout"
                theme_payload = {
                    "theme_code": theme_code,
                    "theme_name": theme.get("theme_name") or theme_code,
                    "direction": theme.get("direction") or str(cluster.get("direction") or "neutral").strip().lower() or "neutral",
                    "horizon": str(cluster.get("horizon") or "swing_5_20d").strip() or "swing_5_20d",
            ...
            raw_events.append({
                ...
                "summary": cls._compact_text(cluster.get("summary") or cluster.get("event_name") or event_id, limit=160),
                "direction": str(cluster.get("direction") or "neutral").strip().lower() or "neutral",
        ...
        raw_events.sort(
            key=lambda item: (
                ...
                max(float((theme.get("score_summary") or {}).get("avg_final_score") or 0.0) for theme in list(item.get("themes") or [{}])),
            ),
            reverse=True,
        )
        payload.update({
    ...
    async def collect(self, db) -> dict:
        ...
        if factor_ic_failures and factor_ic:
            ...
            record_source(
                ...
                details={"expected_factors": ["momentum", "value", "quality", "volatility", "reversal"]},
            )
        ...
        if north_fund_ok:
            record_source("north_fund", "success", ["north_fund_3d_net"], details={"mode": "db_method", "summary": north_summary})
        else:
            record_source("north_fund", "fallback", ["north_fund_3d_net"], reason="north_fund db summary unavailable")
        ...
        if margin_proxy is not None:
            ...
            record_source(
                ...
                details={"mode": "local_proxy", "engine": (event_refresh_summary or {}).get("engine"), "market_internals": local_internals},
            )
        else:
            record_source("margin_data", "fallback", ["margin_5d_change_pct"], reason="margin proxy unavailable")

        hot_sectors = [str(item).strip() for item in list(snapshot.get("hot_sectors") or local_internals.get("hot_sectors") or []) if str(item).strip()]
        cold_sectors = [str(item).strip() for item in list(snapshot.get("cold_sectors") or local_internals.get("cold_sectors") or []) if str(item).strip()]
        ...
        if snapshot["hot_sectors"] or snapshot["cold_sectors"]:
            record_source(
                ...
                details={"mode": "local_rotation", "engine": (event_refresh_summary or {}).get("engine"), "market_internals": local_internals},
            )
        else:
            record_source("sector_fund_flow", "fallback", ["hot_sectors", "cold_sectors"], reason="local sector rotation unavailable")
