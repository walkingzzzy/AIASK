...
@pytest.mark.asyncio
async def test_promotion_pipeline_blocks_high_precision_when_cost_robustness_fails(monkeypatch):
    import akshare_mcp.services.strategy_lifecycle_shared as lifecycle_mod
    ...
    async def _fake_overview(_db, _strategy):
        return {
            ...
            "signal_quality_snapshot": {"status": "strong"},
            "execution_quality_snapshot": {"status": "strong"},
            "position_cycle_evidence": {"status": "candidate", "regime_consistency": 0.9, "cost_robustness": 0.0},
            "regime_validation_summary": {"available": True, "passed": True},
            "cost_robustness_summary": {"available": True, "required": True, "passed": False},
            "trade_density_summary": {"available": True, "passed": True},
            "adverse_regime_avoidance": True,
            "prediction_trace_ledger": {
                "contract_version": "strategy_factory.prediction_trace_ledger.v2",
                "prediction_trace_id": "pred_trace_hp",
                "evidence_gap_codes": [],
            },
            "hard_gate_result": {
                "passed": True,
    ...
    db.get_latest_strategy_incubation_metric = AsyncMock(
        return_value={
            ...
            "sharpe_ratio": 1.15,
            "hit_rate_5d": 0.67,
            "forward_sharpe_5d": 0.55,
        }
    )
    db.get_strategy_incubation_account = AsyncMock(return_value={"account_id": "acct_hp", "stage": "incubating"})
    db.save_strategy_promotion_review = AsyncMock(side_effect=_save_review)
    db.save_strategy_domain_event = AsyncMock()

    result = await StrategyPromotionPipelineService().review(
        db,
        {"id": "sid_hp", "status": "incubating"},
    )

    review = result["review"]
    assert review["status"] == "watch"
    assert review["recommendation"] == "observe"
    assert "high_precision_cost_fragility" in review["blockers"]
    assert review["summary"]["objective_profile"] == "high_precision"
    assert review["summary"]["position_cycle_evidence"]["status"] == "candidate"
