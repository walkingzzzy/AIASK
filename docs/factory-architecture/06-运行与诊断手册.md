...
标准入口（注意：文件名 `run_three_factories.py` 是历史遗留命名，实际启动四个运行体）：
...
注意：
1. 文件名 `run_three_factories.py` 是历史命名债务，实际启动四个运行体（strategy_factory、factor_mining_factory、incubation_factory、market_event_ingest）。
