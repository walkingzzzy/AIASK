...
7. **只读日报（推荐）**：`uv run python scripts/ops/runtime_formal_daily.py` — 输出 formal/observe/orders/trades/`signal_tracker`/`next_actions`，并写 `reports/ops/runtime_formal_daily_YYYY-MM-DD.json`。
8. **共启清单**：`scripts/factories/COSTART_EVIDENCE_LOOP.md`（四工厂 + SignalTracker）。
9. **代码对标现状一页纸**：`docs/CURRENT.md`（排障前先对齐 mature 口径，禁止把 code_closed_loop 当 Live）。
...
若 `runtime_formal_daily` / readiness 出现：

- `bootstrap_factory_runtime` → 先确认 DB 路径与四工厂是否在写同一库  
- `start_signal_tracker_sidecar` → 本 sidecar 未产生信号证据  
