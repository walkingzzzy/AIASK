...
- `configure_strategy_factory_runtime_services()` 当前注册 20+ 个 runtime provider，并为 factor mining、incubation、signal tracker、market event ingest 提供 support/runtime glue。
...
```mermaid
flowchart LR
  subgraph Supervisor["run_three_factories.py (历史命名,实际启动四运行体)"]
    SF["Strategy Factory"]
...
| 入口 | 标准用途 | 规范判断 |
| --- | --- | --- |
| `scripts/factories/run_three_factories.py` | 当前正式四运行体 supervisor (注意：文件名是历史命名，实际启动四个运行体) | 启动 Strategy、Factor Mining、Incubation、Market Event Ingest |
