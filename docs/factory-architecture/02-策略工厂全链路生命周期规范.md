...
`strategies.status` 的当前 transition map 来自 `packages/akshare-mcp/src/akshare_mcp/services/strategy_lifecycle_shared/state_machine.py`（`common.py` 仅 re-export，避免未来漂移需把单一来源固定在 state_machine）：
...
注意：
- 当前只有 `rejected -> draft`，**没有** `draft -> rejected` 双向边。
- 图中必须包含 `listed -> archived`、`suspended -> incubating` 等转换；省略它们会导致排障/报表漏边。
- `published` 会经 `normalize_status_alias` 规范化为 `listed`，图中保留是为兼容旧写入。
...
当前 production floor 默认是 `20`，由 `evaluate_execution_audit_gate(..., production_trade_floor=20)` 给出；该 `20` 是**最低门槛**（production floor），可按策略族收紧，但禁止用 bootstrap/backtest 放宽。bootstrap floor 来自 provisional/backtest thresholds 的较大值，最低为 `1`。因此”bootstrap 不进入 production hard gate”的正确表达是：`bootstrap_pending` 和 `bootstrap_ready` 可以作为诊断/样本债状态出现，但不能被解释为 `passed`。

补充（2026-07-12 起）：`promotion_ready` 与孵化 stage `graduation_ready`（`strategy_lifecycle_shared/incubation.py`）均对 `stability_gap is None` **fail-closed**；signal quality snapshot 的 `strong` 档也已对齐。缺失稳定性缺口不得靠 None 通过晋级/毕业门。
