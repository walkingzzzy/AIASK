...
| `ExecutionUniverseContract` | canonical owner 已落在 `strategy-factory`；当前 SignalTracker 与 Incubation 仍有多条兼容查询路径和部分共享 helper |
...
| P1 | `run_three_factories.py` 命名历史债 | 已记录文件名与四运行体不一致 | 继续保留历史文件名并在文档/守门测试中冻结“文件名是历史债、行为是四运行体”的事实 |
...
| P2 | ExecutionUniverseContract 消费面收口 | canonical owner 已完成 | 继续移除 shared provider 之外的 legacy fallback，并替换两边兼容查询 |
