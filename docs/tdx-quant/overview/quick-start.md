...
> 原始URL: https://help.tdx.com.cn/quant/docs/markdown/mindoc-1cfsjkbf8f3is/mindoc-1cv7o3nje2gu8.html
> 抓取时间: 2026-02-03
...
# 3. 核心：计算当日相较于昨日的涨幅（%）
prev_close = close_df.shift(1)  # 昨日收盘价（向下平移1行）
daily_gain = (close_df - prev_close) / prev_close * 100

# 4. 筛选符合条件的股票（最新交易日涨幅超target_gain%）
latest_date = daily_gain.index[-1]
latest_daily_gain = daily_gain.loc[latest_date]
...
运行后会在控制台输出符合条件的股票列表及其涨幅信息。

### 通达信终端

在通达信客户端中可以看到自定义板块已更新为筛选出的股票。
