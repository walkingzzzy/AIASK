...
> 文档定位：基于当前项目实际代码的完整改造方案，不是理想化蓝图，不把未实现能力写成已完成  
> 结论先行：当前策略工厂满足的是“研究候选持续生成”，还不满足“稳定产出可直接上交易的组合策略”

---

## 一、执行摘要

当前工厂之所以无法满足实际需求，不是因为还漏了几个低质量 snapshot 候选，也不是因为某个 family 阈值不够紧，而是因为系统中段仍然把“研究候选”当成“交易策略”在处理。

最关键的结构性错位有 5 个：

1. `Gate-1` 和 `Gate-2` 验证的不是同一个对象。
2. 多标的候选仍然不是组合级真实回测，而是“逐股票回测后再聚合”的 proxy。
3. `governed_candidate_pool`、`stock_family_allocation`、`factor_scheduler` 已经存在，但还没有成为工厂唯一主驱动。
4. `dedup/refresh` 的身份语义仍然偏“研究任务相似”，而不是“交易对象是否同一策略”。
5. run summary 仍偏“提交数量视角”，没有把 research、incubation、live-ready 三层拆开。

因此，继续沿着 `pipeline_staged rsi / ma_cross / momentum` 这类 family 做微调，最多只能减少坏候选漏入 `Gate-2`，并不能把工厂升级成真实可交易策略工厂。

本方案的核心不是“再缩漏斗”，而是把主链统一到三个一致性上：
...
| 执行现实性仍偏 audit/proxy | `_engine_support.py` 里已有成本、滑点、冲击、持仓约束字段，但成交仿真仍然是单票标量近似 | 回测收益与可执行收益仍有系统性偏乐观风险 | P1 |
| governed pool 不是主驱动 | `cycle_runner.py` 仍固定先跑 `Spawner.spawn()`；`factor_research.py` 有 allocation，但运行链常回退 | 工厂长期停留在 `seed_fallback` / 局部 snapshot 放量模式 | P0 |
| factor scheduler 生命周期不稳定 | scheduler/provider 链上仍可能退到 `local_rule_fallback` | 因子研究和 governed pool 供给不稳定 | P0 |
| bulk lane 与 autonomy 预算互挤 | `_factory_scheduler_loop.py`、`opportunity.py` 仍存在全局预算争抢 | `planned_bulk_task_count` 经常大于 `selected_bulk_task_count` | P0 |
| bulk 展开顺序失衡 | 大量 `ma_cross pass-1` 会先排在前面 | 即使 `stock_family_allocation` 漂亮，运行面也看不到多 family 供给 | P0 |
| dedup/refresh 身份语义过粗 | `deduplicator.py` 主要看 task signature、params similarity、target overlap | 真正有新 target pool / 新验证语义的候选会被打成 `refresh_metrics_only` | P0 |
| summary 口径不对 | `cycle_runner.py` / `quality_reporting.py` 仍偏总 submitted 数 | 团队无法区分“研究产能提升”和“可交易策略产出提升” | P2 |

### 2.3 已经做过、但不能继续当主线的问题
...
1. 候选对象必须在生成阶段就具备完整交易合同，而不是在 `Gate-2` 才临时补齐执行假设。
2. `Gate-1` 和 `Gate-2` 必须验证同一候选合同，只允许“快版”和“完整版”的差异，不允许对象语义不同。
3. 多标的候选必须走真实组合级回测和成交仿真。
4. `Gate-3` 必须严格按 `validation_profile` 分流，不再用因子式统计协议主审交易规则候选。
5. `dedup/refresh/revision` 必须基于交易身份和 lineage 判断，而不是只看研究任务签名。
6. run summary 必须拆为 `research / incubation / live-ready` 三层，且每层有单独指标。
...
### 4.1 新核心对象：`PortfolioCandidateContract`

建议新增统一候选合同，作为生成到 admission 的唯一主对象。
...
## P0：本周必须完成，目标是把主驱动和主门禁对齐

### P0-1 统一候选合同，消灭 Gate-1 / Gate-2 对象错位

**目标**

让 `Pre-Gate`、`Gate-1`、`Gate-2`、`Gate-3` 都围绕同一个 `PortfolioCandidateContract` 工作。
...
- 将当前分散在 candidate 根字段、`portfolio_spec`、`execution_assumptions`、`research_task` 的关键交易字段统一收口。
- `Gate-1` 不再直接拿裸 `strategy_type + params` 跑单票回测，而是读取候选合同的快版验证计划。
- `Gate-2` 的 `FactoryBacktestAssumptions` 改成来自统一合同，而不是二次推导。
...
- governed pool 缺失时，主动触发 scheduler warmup / refresh，而不是等 stale。
- 独立 `factory run` 启动前显式执行一次 scheduler warmup。
- 缺 pool 时把运行态标记成 `refreshing_pool`，而不是继续用 `seed_fallback` 放量。
...
### P0-4 解除 bulk lane 与 autonomy 预算挤占，并重排 bulk 展开顺序

**目标**

既要让 bulk 任务真正展开，又要避免单一 family 抢占整个预算窗口。
...
- `packages/strategy-factory/src/strategy_factory/application/constants.py`
- `.env`

**动作**

- 将 bulk lane 预算与 autonomy 预算拆分。
- `selected_bulk_task_count` 不再受 `MAX_RESEARCH_TASKS=20` 的统一截断。
- bulk task 展开顺序按 `stock_family_allocation`、family diversity、recent failure penalty 排序。
...
- 将 `candidate_family_id`、`validation_profile`、`portfolio_spec`、`target_pool_id`、`lineage_id` 纳入身份计算。
- 刷新判定从“任务相似”升级为“交易对象相同且目标池充分重合”。
- 对 `target_overlap` 不足、但任务签名相近的候选，优先走 `revision` 而不是 `refresh_metrics_only`。
...
- 事件型候选进入 `event_study_validation`，不再复用 trade/factor 逻辑。

**验收**

- event 候选的核心验证指标都能追溯到真实事件样本，而不是回测尾部切窗。

### P1-4 `live_candidate_ready` 升级为真实分流动作

**目标**

把 `live_candidate_ready` 从“报告字段”升级成真正决定去向的动作信号。
...
- 按 `research only / incubation / paper / runtime review / pool admission` 显式分流。
- 每次分流附带触发原因、缺口和回退条件。
...
### P2-1 让 `stock_family_allocation` 成为 bulk lane 主驱动

**目标**

把 allocation 从“报告字段”升级为 bulk 任务主输入。
...
- bulk task 按 allocation 直接生成，而不是先生成统一 task 再让 family 抢占。
...
### P2-3 建正式 multiple testing / lineage registry

**目标**

把挖掘偏差控制从 run-level proxy 升级成正式注册表。
...
- 输出 `research_summary`、`incubation_summary`、`live_ready_summary` 三层。
...
1. 先完成 P0-1 到 P0-4。
   原因：对象不统一、主驱动不稳定、预算互挤时，后面做组合引擎也会建立在错误输入上。

2. 再完成 P0-5、P0-6。
   原因：要先止住坏输入和错误 refresh，避免后续验证层继续吃脏数据。

3. 再进入 P1-1、P1-2、P1-3。
   原因：这是把“研究候选”升级为“交易对象”的决定性改造。

4. 最后做 P2、P3。
   原因：summary、registry、feedback loop 必须建立在稳定主链之上。
...
- run summary 明确拆成 `research / incubation / live-ready`
...
- 继续追求 submitted 数量，却不拆 research/incubation/live-ready
...
1. 新的 `PortfolioCandidateContract` 领域模型与兼容层。
2. governed pool 主驱动运行链。
3. fast portfolio gate 与 full portfolio backtest 两级验证。
4. event study validation 独立协议。
5. 组合级执行现实性仿真。
6. dedup/refresh lineage identity。
7. 三层 run summary 与 live-ready 分流报告。
8. multiple testing / lineage registry。

---

## 十一、最终判断

当前策略工厂无法满足实际需求，不是因为“还有几个坏候选漏进了 `Gate-2`”，而是因为系统主链仍把“研究候选”当成“交易策略”来处理。
