...
- P0：已完成。最小回归集、P0 清单、兼容契约测试均已落库。
- P1：已完成。`packages/strategy-factory/` 包骨架、导入链路、`contracts`、`facade` 已建立。
- P2：已完成。低耦合模块已迁入新包，旧路径保留 compat/re-export。
- P3：已完成。中高耦合模块已迁入新包，默认运行通过 adapter / runtime bridge 与 MCP 交互。
- P4：已完成。调度器主实现已迁入新包，旧单例入口保留兼容语义。
- P5：已完成。MCP 外部调用方已收敛到 `strategy_factory` 公共入口；应用层残留的静态 `akshare_mcp.services.*` 依赖已清理。
- P6：已完成。迁移说明与 compat 定位已同步到代码与文档；当前进入兼容观察期，而非继续大规模搬迁阶段。

当前建议：保持 compat 层至少一个观测周期，除非明确确认历史 patch-point 已无人依赖，否则不要激进删除旧 shim。
