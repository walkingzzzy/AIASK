...
export class AlertsService {
  ...
  async list(status = 'active', userId = 'default'): Promise<AlertsListDto> {
    ...
    try {
      payload = await this.callTool('alerts_manager', args, true);
    } catch (error) {
      fallbackReason = this.extractErrorMessage(error, '告警列表暂不可用');
      payload = null;
    }
    const result: AlertsListDto = {
      status: normalizedStatus,
      items: fallbackReason
        ? this.mergeRecentlyCreated(userId, normalizedStatus, [])
        : this.mergeRecentlyCreated(
          userId,
          normalizedStatus,
  ...
  private extractErrorMessage(error: unknown, fallback: string) {
    ...
    const responseRecord =
      response && typeof response === 'object' && !Array.isArray(response)
        ...
        : {};
    const message = responseRecord.message ?? responseRecord.error ?? (error instanceof Error ? error.message : null);
