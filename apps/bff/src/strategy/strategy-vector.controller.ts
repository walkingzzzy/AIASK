...
export class StrategyVectorController {
  ...
  async vectorReconcile(@Body() body: VectorReconcileDto, @Req() req: Req_) {
    const data = await this.svc.vectorReconcile({
      ...
      limit_profiles: body.limit_profiles,
    });
    return { success: true, data, traceId: tid(req) };
  }
  ...
  async vectorRebuild(@Body() body: VectorRebuildDto, @Req() req: Req_) {
    const data = await this.svc.vectorRebuild({
      index_name: body.index_name,
      index_version: body.index_version,
      statuses: body.statuses,
      limit: body.limit,
      profile_type: body.profile_type,
      vector_method: body.vector_method,
    });
    return { success: true, data, traceId: tid(req) };
  }
  ...
  async vectorHealth(@Query() q: VectorHealthQueryDto, @Req() req: Req_) {
    const data = await this.svc.vectorHealth({
      index_name: q.index_name,
      limit_versions: q.limit_versions,
      include_hnsw_indexes: q.include_hnsw_indexes,
    });
    return { success: true, data, traceId: tid(req) };
  }
  ...
  async vectorCleanup(@Body() body: VectorCleanupDto, @Req() req: Req_) {
    const data = await this.svc.vectorCleanup({
      ...
      limit_versions: body.limit_versions,
      protect_versions: body.protect_versions,
    });
    return { success: true, data, traceId: tid(req) };
  }
  ...
  async rebuildDomainProjection(@Param('id') id: string, @Body() body: DomainProjectionRebuildDto, @Req() req: Req_) {
    const data = await this.svc.rebuildDomainProjection(id, {
      limit: body.limit,
      statuses: body.statuses,
      source: body.source,
    });
  ...
  async rebuildDomainProjections(@Body() body: DomainProjectionRebuildDto, @Req() req: Req_) {
    const data = await this.svc.rebuildDomainProjection(undefined, {
      limit: body.limit,
      statuses: body.statuses,
      source: body.source,
    });
