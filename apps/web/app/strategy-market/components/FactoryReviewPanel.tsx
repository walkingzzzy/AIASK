...
function formatObjectSummary(value: Record<string, unknown> | null | undefined, length = 48) {
  ...
  return shortText(
    entries.map(([key, item]) => `${key}:${Array.isArray(item) ? item.join(',') : String(item)}`).join(' / '),
    length,
  );
}
...
const FACTORY_SECTION_TABS: ReadonlyArray<{ key: FactoryReviewSection; label: string }> = [
...
export function FactoryReviewPanel({
  ...
  }), [domainProjection, events, incubation, latestPromotionReview, latestProjectionSnapshot, runtimeControl]);
  const incubationState = useMemo(() => ({
  ...
  }), [latestRuntimeRiskSnapshot, riskEvents, runtimeAlerts, runtimeControl, runtimeRiskSnapshots]);
  ...
  }), [latestVectorIndexSnapshot, similarProfiles, vectorIndexSnapshots, vectorProfiles]);
  const experimentState = useMemo(() => ({
    experimentRows: aiExperiments.map((item) => {
      const review = (item.evaluation?.committee_review ?? {}) as NonNullable<AiExperiment['evaluation']>['committee_review'];
      const alignmentIssues = Array.isArray(review?.alignment_issues) ? review.alignment_issues : [];
      const executionIssues = Array.isArray(review?.execution_issues) ? review.execution_issues : [];
      const capacityIssues = Array.isArray(review?.capacity_issues) ? review.capacity_issues : [];
      return {
        ...
        score: review?.final_score == null ? '-' : fmtNum(review.final_score, 4),
        review_decision: review?.decision ?? '-',
        review_breakdown: [
          `执行 ${fmtNum(review?.execution_score, 2)}`,
          `容量 ${fmtNum(review?.capacity_score, 2)}`,
          `对齐 ${fmtNum(review?.task_alignment_score, 2)}`,
          `新颖 ${fmtNum(review?.novelty_score, 2)}`,
        ].join(' / '),
        ...
        rank: review?.rank ?? '-',
        champion: review?.is_champion ? '是' : '否',
        status: item.status ?? '-',
  ...
  }), [aiExperiments, domainEvents, taskRuns]);
  const reviewAuditRows = useMemo(() => {
    ...
    return [
      ...
      { item: '约束审计', value: formatIssueSummary([
      ...
      { item: '尝试惩罚', value: formatIssueSummary([
      ...
      { item: '任务偏好', value: formatIssueSummary([
