...
import { BarChart, COLORS, LineChart, PieChart } from '@/components/charts';
import { StockLink } from '@/components/stock-link';
import { EmptyState } from '@/components/status-state';
import { DataTable, KpiCard, KpiGrid, SectionCard, StockCodeInput } from '@/components/ui';
import { fmt } from '@/lib/api';
import { extractArray, extractObject, fmtAmount, fmtNum, fmtPct } from '@/lib/data-utils';
import { exportCSV } from '@/lib/export';
import {
  fundFlowChipButtonCls,
  fundFlowLinkChipCls,
  fundFlowPrimaryButtonCls,
} from '@/app/fund-flow/components/fund-flow-panel-styles';
import type { FundFlowTab } from '@/app/fund-flow/lib/fund-flow-view';

type FundFlowTabPanelsProps = {
  tab: FundFlowTab;
  loading: boolean;
  code: string;
  ...
  dragonData: unknown;
  marginData: unknown;
  marginRankData: unknown;
  blockTradesData: unknown;
  northHoldingData: unknown;
  northTopData: unknown;
  setCode: (value: string) => void;
  setTab: (value: FundFlowTab) => void;
  validate: () => boolean;
  loadStockFlow: (nextCode?: string) => void;
  loadMarginData: (nextCode?: string) => void;
  loadNorthDetail: (nextCode?: string) => void;
  loadSectorFlow: () => void;
  loadConceptFlow: () => void;
  loadNorthFlow: () => void;
  loadDragonTiger: () => void;
  loadMarginRanking: () => void;
  loadBlockTrades: () => void;
};

export default function FundFlowTabPanels({
  tab,
  loading,
  code,
  codeError,
    ...
    <>
      {tab === 'stock' && (
        <SectionCard tabAttached>
          ...
          {stockData ? (
            (() => {
              const items = extractArray(stockData, 'flows');
              return items.length ? (
                <>
                  <BarChart
      ...
      {tab === 'sector' && (
        <SectionCard tabAttached>
          <button type="button" disabled={loading} onClick={loadSectorFlow}>
            加载板块资金流
          </button>
          {sectorData ? (
            (() => {
              const rows = extractArray(sectorData, 'flows');
              const top5In = rows
            ...
            <EmptyState
              ...
              action={
                <>
                  <button type="button" onClick={loadSectorFlow} className={fundFlowPrimaryButtonCls}>
                    立即加载板块榜单
                  </button>
      ...
      {tab === 'concept' && (
        <SectionCard tabAttached>
          <button type="button" disabled={loading} onClick={loadConceptFlow}>
            加载概念资金流
          </button>
          {conceptData ? (
            (() => {
              const rows = extractArray(conceptData, 'flows');
              return rows.length ? (
                <DataTable
                  rows={rows}
                  columns={[
            ...
            <EmptyState
              ...
              action={
                <>
                  <button type="button" onClick={loadConceptFlow} className={fundFlowPrimaryButtonCls}>
                    立即加载概念榜单
                  </button>
      ...
      {tab === 'north' && (
        <SectionCard tabAttached>
          <button type="button" disabled={loading} onClick={loadNorthFlow}>
            加载北向资金
          </button>
          {northData ? (
            (() => {
              const items = extractArray(northData, 'items', 'flows');
              return items.length ? (
                <LineChart
            ...
            <EmptyState
              ...
              action={
                <>
                  <button type="button" onClick={loadNorthFlow} className={fundFlowPrimaryButtonCls}>
                    加载北向走势
                  </button>
                  <button type="button" onClick={() => setTab('north-detail')} className={fundFlowChipButtonCls}>
                    去看北向明细
                  </button>
                </>
              }
      ...
      {tab === 'dragon' && (
        <SectionCard tabAttached>
          <button type="button" disabled={loading} onClick={loadDragonTiger}>
            加载龙虎榜
          </button>
          {dragonData ? (
            (() => {
              const rows = extractArray(dragonData);
              return rows.length ? (
                <DataTable
                  rows={rows}
                  columns={[
            ...
            <EmptyState
              ...
              action={
                <>
                  <button type="button" onClick={loadDragonTiger} className={fundFlowPrimaryButtonCls}>
                    加载龙虎榜
                  </button>
                  <Link href="/stock" className={fundFlowLinkChipCls}>
                    去个股页核对
                  </Link>
                </>
              }
        ...
        <SectionCard tabAttached>
          <div className="flex gap-3 flex-wrap items-end">
            <StockCodeInput
              ...
              placeholder="留空看全市场"
            />
            <button type="button" disabled={loading} onClick={() => loadMarginData()}>
              查询融资融券
            </button>
          </div>
          <p className="mt-2 text-sm text-text-secondary">
          ...
          {marginData ? (
            (() => {
              const rows = extractArray(marginData);
              return (
                <>
                  {rows.length > 1 && (
                    <LineChart
          ...
          <div className="mt-3">
            <button type="button" disabled={loading} onClick={loadMarginRanking}>
              融资融券排名
            </button>
            {marginRankData
              ? (() => {
                  const rows = extractArray(marginRankData);
                  return rows.length ? (
                    <DataTable
                      rows={rows}
                      columns={[
      ...
      {tab === 'block-trades' && (
        <SectionCard tabAttached>
          <button type="button" disabled={loading} onClick={loadBlockTrades}>
            加载大宗交易
          </button>
          {blockTradesData ? (
            (() => {
              const rows = extractArray(blockTradesData);
              return rows.length ? (
                <DataTable
                  rows={rows}
                  columns={[
            ...
            <EmptyState
              ...
              action={
                <>
                  <button type="button" onClick={loadBlockTrades} className={fundFlowPrimaryButtonCls}>
                    加载大宗交易
                  </button>
                  <Link href="/research" className={fundFlowLinkChipCls}>
                    去研究页联动分析
      ...
      {tab === 'north-detail' && (
        <SectionCard tabAttached>
          ...
          {northHoldingData || northTopData ? (
            <>
              {northHoldingData &&
                (() => {
                  const obj = extractObject(northHoldingData);
              ...
              {northTopData &&
                (() => {
                  const rows = extractArray(northTopData);
                  return rows.length ? (
                    <DataTable
                      rows={rows}
                      columns={[
