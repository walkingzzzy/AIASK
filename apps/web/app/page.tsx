...
export default function HomePage() {
  ...
  /* ── Data queries ─────────────────────────────────────────────── */
  const idxQ       = useApiQuery<unknown>('/market/batch-quotes', { enabled: mounted, refetchInterval: liveRefetch, placeholderData: 'keepPrevious', body: { codes: INDEX_CODES }, parse: (r) => ensureRecordOrArray(r, '首页指数批量') });
  ...
  const paperSumQ  = useApiQuery<Record<string, unknown>>('/paper-trading/summary', { enabled: mounted, parse: (r) => ensureRecord(r, '模拟盘概览(首页)') });
  const paperPosQ  = useApiQuery<unknown>('/paper-trading/positions', { enabled: mounted, parse: (r) => ensureRecordOrArray(r, '模拟盘持仓(首页)') });
  const newsQ      = useApiQuery<unknown>('/research/market-news?limit=5', { enabled: mounted, parse: (r) => ensureRecordOrArray(r, '市场快讯(首页)') });
  ...
  const alertsQ     = useApiQuery<unknown>('/alerts/list?status=active', { enabled: mounted && dashboardVisibility.alerts, parse: (r) => ensureRecordOrArray(r, '活跃预警(首页)') });
  ...
  const strategySubsQ = useApiQuery<unknown>(
    ...
    { enabled: mounted && dashboardVisibility.strategy && Boolean(strategyUserId), parse: (r) => ensureRecordOrArray(r, '策略订阅(首页)') },
  );
  ...
  const handleWsQuote = useCallback((data: QuoteData) => { wsQuotesRef.current.set(data.code, data as Record<string, unknown>); setWsQuoteTick((t) => t + 1); }, []);
  ...
  const quoteMap = useMemo(() => { const m = new Map<string, Record<string, unknown>>(); extractArray(batchQ.data, 'quotes', 'items', 'data').forEach((q) => { const c = String(q.code ?? ''); if (c) m.set(c, q); }); return m; }, [batchQ.data]);
  ...
  const validIndices = useMemo(() => indices.filter((o) => Number.isFinite(Number(o.price ?? o.close ?? o.current))), [indices]);
  ...
  const paperSummary = useMemo(() => extractObject(paperSumQ.data), [paperSumQ.data]);
  const paperAccount = useMemo(() => extractObject(paperSummary.account), [paperSummary]);
  const paperPositions = useMemo(() => extractArray(paperPosQ.data, 'positions', 'items', 'data'), [paperPosQ.data]);
  const marketNews = useMemo(() => extractArray(newsQ.data, 'items', 'news', 'data'), [newsQ.data]);
  ...
  const quickActions = useMemo(() => {
    const dc = recentStocks[0]?.code || watchlistItems[0]?.code || '600519';
    return [
  ...
  const marketAnomalies = useMemo(() => {
    ...
    type Anomaly = { title: string; value: string; href: string; tone: 'danger' | 'success' | 'info' | 'warning' };
    const out: Anomaly[] = [];
  ...
  const dashboardCards: DashboardCard[] = useMemo(() => [
    {
      key: 'risk', title: '风险巡检', pending: riskQ.isPending, error: riskQ.error, empty: !riskSummary.lookbackDays, href: '/risk?lookbackDays=252&from=home',
      ...
      footer: <div className="mt-2 text-xs text-text-secondary">VaR: {riskMs.var?.ok === false ? '异常' : '正常'} | 压测: {riskMs.stress?.ok === false ? '异常' : '正常'} | 暴露: {riskMs.exposure?.ok === false ? '异常' : '正常'}</div>,
    },
    {
      key: 'strategy', title: '策略动态', pending: strategyUserId ? strategySubsQ.isPending : false, error: strategyUserId ? strategySubsQ.error : null, empty: strategyUserId ? strategySubs.length === 0 : false, href: '/strategy-market?from=home',
      content: <KpiGrid cols={3}><KpiCard title="订阅策略" value={strategySubs.length} /><KpiCard title="活跃用户" value={user?.username ?? '-'} /><KpiCard title="状态" value={strategyUserId ? (strategySubs.length > 0 ? '已订阅' : '待订阅') : '未登录'} /></KpiGrid>,
      footer: strategyUserId ? (strategySubs.length > 0 ? <div className="mt-2 text-xs text-text-secondary">最近订阅：{String(strategySubs[0]?.name ?? strategySubs[0]?.strategy_name ?? '-')}</div> : null) : <div className="mt-2 text-xs text-text-secondary">登录后可查看你的订阅策略动态</div>,
    },
    {
  ...
  ], [riskQ.isPending, riskQ.error, riskSummary, riskDegraded, riskMs, strategySubsQ.isPending, strategySubsQ.error, strategySubs, strategyUserId, user, alertsQ.isPending, alertsQ.error, activeAlerts]);
  ...
  return (
    <PageContainer>
      ...
      />
      <MarketOverview
        dateStr={dateStr} lastUpdated={lastUpdated} fgValue={fgValue} luStats={luStats}
