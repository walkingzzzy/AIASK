        ...
        <div className="space-y-3">
          ...
          <div className="pt-2 border-t border-glass-border space-y-2">
            ...
            <label htmlFor="settings-old-password" className="block">
              ...
              <input id="settings-old-password" type="password" value={oldPassword} onChange={(e) => setOldPassword(e.target.value)} className="w-full border border-border rounded px-3 py-2 bg-surface text-sm" placeholder="输入当前密码" />
            </label>
            <label htmlFor="settings-new-password" className="block">
              <span className="text-xs text-text-secondary mb-1 block">新密码</span>
              <input id="settings-new-password" type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full border border-border rounded px-3 py-2 bg-surface text-sm" placeholder="至少 6 位" />
            </label>
            <label htmlFor="settings-confirm-password" className="block">
              <span className="text-xs text-text-secondary mb-1 block">确认新密码</span>
              <input id="settings-confirm-password" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="w-full border border-border rounded px-3 py-2 bg-surface text-sm" placeholder="再次输入新密码" />
            </label>
            <button type="button" onClick={() => void changePassword()} disabled={passwordApi.isPending} className="px-4 py-2 rounded border border-primary/40 text-primary text-sm cursor-pointer disabled:opacity-50">{passwordApi.isPending ? '提交中...' : '修改密码'}</button>
          </div>
          <div className="pt-2 border-t border-glass-border space-y-2">
