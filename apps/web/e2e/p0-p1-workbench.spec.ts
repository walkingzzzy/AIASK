...
async function resolveFirstStrategyDetailPath(page: Page) {
  const detailHref = await page.locator('a[href^="/strategy-market/"]').evaluateAll((nodes) => {
...
async function waitForScreenerSettled(page: Page, timeoutMs = 20_000) {
  await expect
    .poll(async () => {
      if (await page.getByText(/筛选结果（\d+ 只）/).isVisible().catch(() => false)) return 'results';
      if (await page.getByText('暂无筛选结果，请输入选股条件后点击执行').isVisible().catch(() => false)) return 'empty';
      if (await page.getByText(/HTTP \d+|validation error|error executing tool/i).isVisible().catch(() => false)) return 'error';
      return null;
...
test('should complete P0-P1 workbench flows without critical regressions', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: /事件日历(?:工作台)?/, level: 1 })).toBeVisible();
  ...
  const workspaceToggle = page.getByRole('button', { name: /展开工作区设置|收起工作区设置/ }).first();
  await expect(workspaceToggle).toBeVisible();
  if ((await workspaceToggle.getAttribute('aria-expanded')) !== 'true') {
    await workspaceToggle.click();
  }

  const viewNameInput = page.getByLabel('工作区视图名称');
  ...
  await Promise.all([
    page.waitForURL(/\/execution(?:\?.*)?$/),
    page.getByRole('button', { name: '联动执行中心', exact: true }).click(),
  ]);
  await dismissOnboarding(page);
  await expect(page.getByRole('button', { name: '提交执行' })).toBeVisible({ timeout: 20_000 });
  ...
  await executionForm.getByLabel('artifact_id').fill(artifactId);
  ...
  await expect(page.getByRole('button', { name: '打开 artifact 详情页面板', exact: true })).toBeVisible();

  await Promise.all([
    page.waitForURL(/\/execution\/artifacts\/art_pw_exec_/),
    page.getByRole('button', { name: '打开 artifact 详情页面板', exact: true }).click(),
  ]);
  ...
  await Promise.all([
    page.waitForURL(/\/execution\?(?=.*artifact_id=art_pw_exec_).*/),
    page.getByRole('button', { name: '执行中心', exact: true }).click(),
  ]);
  await expect(page.getByRole('button', { name: '提交执行' })).toBeVisible({ timeout: 20_000 });

  await Promise.all([
    page.waitForURL(/\/performance\?(?=.*mode=account).*/, { timeout: 20_000 }),
  ...
  await dismissOnboarding(page);
  await expect(page.getByRole('heading', { name: /绩效复盘工作台|绩效中心/ })).toBeVisible();
  ...
  await waitForScreenerSettled(page);
  ...
  await expect(page.getByText(/筛选结果（\d+ 只）/)).toBeVisible({ timeout: 20_000 });
  ...
  await expect(page.locator('article').filter({ hasText: firstWatchCode }).first()).toBeVisible();
  ...
  const strategyDetailPath = await resolveFirstStrategyDetailPath(page);
  const strategyDetailLink = await firstVisible(page.locator(`a[href="${strategyDetailPath}"]`));
  await expect(strategyDetailLink).toBeVisible();
  ...
  await openProtectedPage(page, strategyDetailPath);
  await expect(page).toHaveURL(/\/strategy-market\/[^/?#]+/);
  await expect(page.getByRole('tab', { name: '策略概览' })).toBeVisible({ timeout: 20_000 });
  await expect(page.getByRole('heading', { level: 1 }).first()).toBeVisible();

  const subscribeButton = page.getByRole('button', { name: /策略头图(?:取消订阅|订阅策略)/ });
  const subscribeLabel = await subscribeButton.textContent();
  await subscribeButton.click();
  await expect(
    page.getByRole('button', {
      name: subscribeLabel?.includes('取消') ? '策略头图订阅策略' : '策略头图取消订阅',
    }),
  ).toBeVisible();
